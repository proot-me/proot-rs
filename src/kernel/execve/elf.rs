...
impl<T> ParameterizedElfHeader<T> {
    ...
    pub fn is_known_phentsize(&self) -> Result<()> {
        let program_header_size = mem::size_of::<ProgramHeader<T>>() as u16;
...
macro_rules! get {
    ($elf_header:expr, $field:ident, $result_type:ty)    => {
            {
                $elf_header.unwrap().apply(
                    |header32| Ok(header32.$field as $result_type),
                    |header64| Ok(header64.$field as $result_type)
                )
            }
        };
    ($elf_header:expr, $field:ident)    => {
            {
                $elf_header.unwrap().apply(
...
mod tests {
    ...
    fn test_get_elf_header_class() {
        assert!(get_elf_header_class(&PathBuf::from("/bin/sleep")).is_ok());
    }

    #[test]
    fn test_extract_elf_header() {
        let result = extract_elf_head(&PathBuf::from("/bin/sleep"));
